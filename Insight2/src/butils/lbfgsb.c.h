/* 
   ...
   2015-10-09 -- Brad Gulko
	fixed some compile warnigs due to use of uninitialized variables
	Push & pop of min/max macros to coexist with c++ limits:: module max()
	Added referece to FLOAT constant to avoid having to recalculate system constants dynamically.
...
// use static system cosntants, rather than calculating them om the fly...
#define USE_HEADER

// Prevent conflicts with other macros in thsi nameslace (like the c++ limits which use max())
...
const static double b173 = .001; /* ftol default for dcsrch */
const static double b174 = .9;   /* gtol default for dcsrch */
const static double b175 = .1;   /* xtol default for dcsrch */
const static double b176 = 1.0e-15; /* min. step value: was 0. */
...
// some of these values are saved in the fitst pass before they are set.
// the C++ compiler complaines abous use before set, so initizlize them.
...
{
    double d1, d2, gd=0, dr=0, rr=0, dtd = 0;
    double tol, stp=0, cpu1 = 0, cpu2 = 0;
    double ddum, fold=0;
    double time1, time2;
    double theta=0, dnorm = 0;
    double xstep=0, stpmx = 0;
    double cachyt=0, epsmch=0;
    double sbtime=0.0, sbgnrm=0.0;
    double lnscht=0, gdold = 0;
